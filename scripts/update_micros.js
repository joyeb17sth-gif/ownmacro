...
for (const [id, micros] of Object.entries(additions)) {
  // Regex to match the object definition of the food
  // We look for: id: 'starter-...', name: '...', category: '...' \n    servingSize: ... \n
  // And optionally the existing micronutrients lines before the closing `  },`
  
  const escapedId = id.replace(/-/g, '\\\\-');
  const regex = new RegExp(`(id:\\s*'${escapedId}'.*?\\n.*?\\n)(?:\\s+vitaminA:.*?\\n)?(?:\\s+vitaminD:.*?\\n)?`, 's');
...
console.log('Constants successfully updated with full micronutrients.');
